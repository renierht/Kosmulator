...
def make_CornerPlot(Samples, CONFIG, model_name, save_file_name, PLOT_SETTINGS):
    ...
    g.settings.legend_fontsize = leg_size + 8

    base = PLOT_SETTINGS.get("label_font_size", 18)
    tick = PLOT_SETTINGS.get("tick_font_size", 14)
    legend = PLOT_SETTINGS.get("legend_font_size", 30)

    legend_fontsize=24

    g.settings.fontsize = 20#base
    g.settings.axes_labelsize = 25#base
    g.settings.axes_fontsize =  25#tick
    g.settings.legend_fontsize = 25#legend
    g.settings.title_limit_fontsize = legend
   ...
   # Choose legend layout: small → 1 row, medium → ~2 rows, large → ~3 rows
    ...
    os.makedirs(folder, exist_ok=True)
    out_path = os.path.join(folder, f"corner_{save_file_name}.pdf")
    fig.savefig(out_path, dpi=int(PLOT_SETTINGS.get("dpi", 600)), bbox_inches="tight")
    plt.close(fig)
