...
################################Roobert added package############################
from classy import Class
#################################################################################


def model_likelihood(theta, obs_data, obs_type, CONFIG, MODEL_func, obs, obs_index, model_name=None):
    """
    ...
    if obs== "CMB_TT":
        l = obs_data['l']
        Dl = obs_data['Dl']
        Dl_minus = obs_data['Dl_minus']
        Dl_plus = obs_data['Dl_plus']
        chi = SP.Calc_cmbtt_chi(param_dict, l, Dl, Dl_minus, Dl_plus,model_name)
        return -0.5 * chi
    ...
    if obs== "CMB_EE":
        ...
        Dl_minus = obs_data['Dl_minus']
        Dl_plus = obs_data['Dl_plus']
        chi = SP.Calc_cmbee_chi(param_dict, l, Dl, Dl_minus, Dl_plus,model_name)
        return -0.5 * chi
    ...
    if obs== "CMB_TE":
        ...
        Dl_minus = obs_data['Dl_minus']
        Dl_plus = obs_data['Dl_plus']
        chi = SP.Calc_cmbte_chi(param_dict, l, Dl, Dl_minus, Dl_plus,model_name)
        return -0.5 * chi
    ################################################################################
...
def lnprob(theta, data, Type, CONFIG, MODEL_func, obs, obs_index,model_name):
    """
    ...
    total_likelihood = sum(
        model_likelihood(theta, data[obs_name], obs_type, CONFIG, MODEL_func, obs_name, obs_index,  model_name=model_name)
...
def run_mcmc(data, model_name="LCDM", chain_path=None, MODEL_func=None,
    ...
    result = op.minimize(
        nll, theta_init,
        args=(data[obs_str], obs_type_for_opt, CONFIG, MODEL_func, obs_str, obs_index,model_name),
        bounds=bnds,
    ...
    if parallel:
        if external_pool:
            ...
            if saveChains:
                ...
                sampler = emcee.EnsembleSampler(CONFIG["nwalker"], CONFIG["ndim"][obs_index], lnprob, args=(data, Type, CONFIG, MODEL_func, obs, obs_index,model_name), backend=backend, pool=pool)
            else:
                sampler = emcee.EnsembleSampler(CONFIG["nwalker"], CONFIG["ndim"][obs_index], lnprob, args=(data, Type, CONFIG, MODEL_func, obs, obs_index,model_name), pool=pool)
        else:
