...
def run_mcmc_for_all_models(models, observations, CONFIG, data, overwrite, convergence, PLOT_SETTINGS, use_mpi, num_cores, suffix="", pool=None, pantheon_cov=None):
    """
    ...
    for j, model_name in enumerate(CONFIG[list(models.keys())[0]]['model_name']):
        ...
        ########################Robert added code################################
            if "CMB_TT" in obs or "CMB_EE" in obs or "CMB_TE" in obs:
                #global_model_name = model_name
                CR.run_model(model_name)#Runs classy for speciic model
        ...
        #########################################################################
            ...
            if not overwrite and os.path.exists(chain_path):
                ...
                print(f"[DEBUG] Samples for label '{label}': {Samples[label]}")            
            if "PantheonP" in obs:
                data["PantheonP"].pop("cov", None)
