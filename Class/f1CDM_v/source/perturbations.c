...
int perturbations_einstein(
  ...
  double k2,a,a2,a_prime_over_a, alpha_ft, F_prime_0, F_doubleprime_0;
  double s2_squared;
  double shear_g = 0.;
  double shear_idr = 0.;
  ...
  alpha_ft = (pba->Omega0_lambda*pow(6*pow(pba->H0,2),1-pba->n_fT))/(1 - 2*pba->n_fT);
  F_prime_0 = -alpha_ft * pba->n_fT * pow(6 * pow(ppw->pvecback[pba->index_bg_H], 2), pba->n_fT - 1);
  F_doubleprime_0 = -alpha_ft * pba->n_fT*(pba->n_fT - 1) * pow(6 * pow(ppw->pvecback[pba->index_bg_H], 2), pba->n_fT - 2);
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == newtonian) {
      ...
      ppw->pvecmetric[ppw->index_mt_psi] = y[ppw->pv->index_pt_phi] - (4.5/(1+ F_prime_0)) * (a2/k2) * ppw->rho_plus_p_shear;

      /* equation for phi' */
      ppw->pvecmetric[ppw->index_mt_phi_prime] = -a_prime_over_a* a * ppw->pvecmetric[ppw->index_mt_psi] + (1.5/(12*(pow(a_prime_over_a,2)/pow(a,2))*F_doubleprime_0 - (1 + F_prime_0))) * (a2/k2) * ppw->rho_plus_p_theta;
