...
int background_functions(
  ...
  double dp_dloga;
  double n_fT;
  double H_temp, H2_temp, alpha_ft, F_prime_0, F_doubleprime_0;
  ...
  /* -----------------------------------------------------------
   Exact f(T) Friedmann solver
   Solves:

   E^2 = Omega_std + Omega_Lambda * E^(2n)

   where

   Omega_std = rho_std / H0^2
------------------------------------------------------------ */
  ...
  rhs = rho_std/(pba->H0*pba->H0);

  /* LCDM initial guess */
  E = sqrt(rhs + pba->Omega0_lambda);

  for (iter = 0; iter < 60; iter++) {

      /* Prevent singular derivative */
      if (E < 1.e-14)
          E = 1.e-14;
      ...
      f =
          E*E
          - rhs
          - pba->Omega0_lambda*pow(E,2.0*pba->n_fT);

      df =
          2.0*E
          - pba->Omega0_lambda
            *(2.0*pba->n_fT)
            *pow(E,2.0*pba->n_fT-1.0);
  ...
  class_test(
      (!isfinite(E)) || (E <= 0.0),
      pba->error_message,
      "Exact f(T) Newton solver failed."
  );

  /* Convert back to H */
  pvecback[pba->index_bg_H] = pba->H0 * E;


  /** - compute derivative of H with respect to conformal time */
  //pvecback[pba->index_bg_H_prime] = - (3./2.) * (rho_tot + p_tot) * a + pba->K/a;

  H_temp = pvecback[pba->index_bg_H];
  H2_temp = H_temp * H_temp;

  alpha_ft = (pba->Omega0_lambda * pow(6.0 * pow(pba->H0, 2), 1 - pba->n_fT)) / (1 - 2 * pba->n_fT);
  F_prime_0 = -alpha_ft * pba->n_fT * pow(6.0 * H2_temp, pba->n_fT - 1);
  F_doubleprime_0 = alpha_ft * pba->n_fT * (pba->n_fT - 1) * pow(6.0 * H2_temp, pba->n_fT - 2);

  pvecback[pba->index_bg_H_prime] = - (3.0/2.0) * (rho_tot + p_tot) * a / (1.0 + F_prime_0 - 12.0 * F_doubleprime_0 * H2_temp);
